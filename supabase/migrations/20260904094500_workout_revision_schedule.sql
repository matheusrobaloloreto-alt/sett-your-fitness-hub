-- Preserve the trainer-selected weekly schedule when creating a new atomic
-- workout revision. The first revision of this RPC used payload ordinality as
-- day_of_week, which could compress schedules such as Mon/Wed/Fri to Mon/Tue/Wed.
...
begin
  ...
  loop
    ...
    v_day_of_week := case
      when v_item ? 'day_of_week' and jsonb_typeof(v_item->'day_of_week') = 'number'
        then (v_item->>'day_of_week')::integer
      else v_index
    end;
    if v_title is null
      or jsonb_typeof(coalesce(v_item->'exercises', 'null'::jsonb)) is distinct from 'array'
      or v_day_of_week < 1
      or v_day_of_week > 7 then
      raise exception 'workout_revision_payload_invalid';
    end if;
