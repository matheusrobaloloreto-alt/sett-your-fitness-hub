...
begin
  ...
  if v_enrollment.id is null then
    ...
    select coalesce(max(tc.cycle_number), 0)
      into v_cycle_number
    from public.training_cycles tc
    where tc.enrollment_id = v_enrollment_id;

    v_cycle_number := v_cycle_number + 1;
    -- Renewal cycles follow the financial term, never a stale/imported cycle tail.
    v_cycle_start := v_extension_start + 1;
...
comment on function public.apply_paid_payment_lifecycle(uuid, uuid, uuid, text, date, date) is
  'Applies a paid lifecycle exactly once; renewals start after the financial term and ignore stale historical cycle tails.';
