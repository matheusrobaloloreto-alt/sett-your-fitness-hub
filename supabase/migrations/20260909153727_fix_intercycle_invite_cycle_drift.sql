...
begin
  ...
  if coalesce(new.status, '') in ('cancelled','superseded')
    ...
    set status = 'cancelled',
      cancelled_at = coalesce(cancelled_at, now()),
      last_error_code = 'cycle_cancelled_or_rescoped_while_sending',
      updated_at = now()
    where training_cycle_id = new.id
      and status = 'sending';
    return new;
  ...
  if new.start_date is distinct from old.start_date or new.end_date is distinct from old.end_date then
    ...
    set scheduled_for = greatest(v_scheduled_for, now()),
      status = case
        when status = 'ready' then 'cancelled'
        when status = 'failed' then 'scheduled'
        else status
      end,
      cancelled_at = case when status = 'ready' then coalesce(cancelled_at, now()) else cancelled_at end,
      next_attempt_at = null,
      last_error_code = case
        when status = 'ready' then 'cycle_rescheduled_manual_link_invalidated'
        else 'cycle_rescheduled'
      end,
      updated_at = now()
    where training_cycle_id = new.id
      and status in ('ready','scheduled','failed');
    ...
    set status = 'cancelled',
      cancelled_at = coalesce(cancelled_at, now()),
      last_error_code = 'cycle_rescheduled_while_sending',
      updated_at = now()
    where training_cycle_id = new.id
      and status = 'sending';
  end if;
...
revoke all on function public.reconcile_intercycle_delivery_for_cycle_change()
...
begin
  ...
  if v_today < v_cycle.start_date + 28
    or v_today > coalesce(v_cycle.end_date, v_cycle.start_date + 41) then
    raise exception 'intercycle_submit_window_closed';
  end if;
