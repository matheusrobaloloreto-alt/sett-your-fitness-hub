-- Fail-closed repair draft for the safe subset of BN current/future cycle overlaps.
--
-- Scope:
-- - enrollment 2d93f08d4f16: shift unused future boundary starts by +1 day.
-- - enrollment 078e2d2b9409: trim one proven-empty placeholder cycle before the canonical cycle.
-- - enrollment 93c4213741e0: shift unused future boundary starts by +1 day.
--
-- Explicitly out of scope:
-- - enrollment 5f9de190f3c7.
-- - enrollments 3530d245ed22, 467cc79d6c5b, 7975a4d98a88.
-- - enrollment d735907ca436 unless a separate alias/template QA clears it.
-- - enrollments.end_date, payments, Asaas records, workouts and workout logs/sessions.
--
-- Do not apply before root/independent QA approves the dry-run output.
...
create table if not exists public.training_cycle_safe_overlap_repair_audit (
...
alter table public.training_cycle_safe_overlap_repair_audit enable row level security;
revoke all on table public.training_cycle_safe_overlap_repair_audit from public, anon, authenticated;
grant select, insert, update on table public.training_cycle_safe_overlap_repair_audit to service_role;
...
declare
  ...
  v_expected_batch_sha256 constant text := '8d79773289aef338a89b5f0dc2c9d917f810625ce33d316364725efb5b2d728a';
  v_count integer;
...
begin
  ...
  if exists (
    ...
    raise exception 'bn_safe_cycle_duplicate_target';
  end if;
  ...
  if v_expected_batch_sha256 <> v_expected_beforeimagehash then
    raise exception 'bn_safe_cycle_internal_constant_mismatch';
  end if;
end
$manifest_gate$;
...
insert into public.training_cycle_safe_overlap_repair_audit (
  repair_key,
  batch_sha256,
  action,
  company_id,
...
begin
  ...
  if v_count <> 6 then
    raise exception 'bn_safe_cycle_audit_insert_mismatch expected=6 actual=%', v_count;
  end if;
end
$audit_gate$;
...
update public.training_cycle_safe_overlap_repair_audit audit
