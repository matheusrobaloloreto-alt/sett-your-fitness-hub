-- Narrow the workout-log set type contract to W/N/F. Drop-set remains an
-- exercise method and is no longer a persisted set_type.
--
-- Online safety: CREATE OR REPLACE preserves the signature, SECURITY DEFINER,
-- search_path, tenant checks, row-lock/CAS behavior and grants. No table scan,
-- data rewrite, backfill or index is required. Existing historical rows are not
-- changed. Deploy this migration before the frontend/edge release.
-- Rollback: restore the immediately previous function definition from
-- 20260825173000_fix_workout_log_payload_alias.sql; no row rewrite is needed.
...
begin
  ...
  loop
    ...
    if item_set_type not in ('warmup', 'normal', 'failure') then
