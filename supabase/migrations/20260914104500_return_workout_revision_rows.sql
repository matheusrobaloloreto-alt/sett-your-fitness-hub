-- Return the confirmed row identities for the new revision. The frontend uses
-- id + updated_at as the next optimistic-concurrency snapshot.
...
declare
  ...
  v_workout_rows jsonb := '[]'::jsonb;
  v_item jsonb;
  v_index integer;
  v_day_of_week integer;
  v_workout_id uuid;
  v_workout_updated_at timestamptz;
  v_title text;
begin
  ...
  loop
    ...
    returning id, updated_at into v_workout_id, v_workout_updated_at;

    v_workout_ids := array_append(v_workout_ids, v_workout_id);
    v_workout_rows := v_workout_rows || jsonb_build_array(jsonb_build_object(
      'id', v_workout_id,
      'updated_at', v_workout_updated_at
    ));
  end loop;
