...
O rollback anterior não era executável sob os triggers reais porque tentava restaurar `updated_at` e depois exigia o hash completo do before-image. Os triggers ativos sobrescrevem esse campo com `now()` em todo `UPDATE`. Os scripts agora preservam o gate exato de after-image antes da escrita, restauram apenas campos de negócio e validam o estado final ignorando somente o timestamp controlado pelo trigger.
...
O segundo lote está bloqueado por atividade operacional posterior em um ciclo existente. A classificação agregada apontou `cycle_timestamp_rows=1` e zero em workout, workout log e sessão; nenhuma identidade foi exposta. O guard fail-closed funcionou: after-image de matrícula/aluno ainda é exato, mas isso não basta para autorizar a reversão diante de atividade dependente posterior.
...
Limite residual: `training_cycles` e `workout_sessions` não têm um `updated_at` universal. O guard usa `created_at`, `superseded_at`, `prescribed_offline_at`, `started_at` e `completed_at`, além de `workouts.updated_at` e `workout_logs.updated_at`, mas não consegue reconstruir alterações históricas sem timestamp que o apply original não persistiu. Por isso, o rollback real continua condicionado a clone/staging e QA, mesmo com readiness `true`.
...
- guard fail-closed adicionado para bloquear atividade dependente posterior ao reparo, com locks contra corrida;
