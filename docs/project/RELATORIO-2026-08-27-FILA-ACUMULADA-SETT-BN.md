...
| Gate TypeScript completo | ❌ (aguardando) | `npx tsc -b` permanece vermelho no `origin/main` com 101 linhas de erros preexistentes. A comparação normalizada entre `origin/main` e este release foi idêntica, portanto a telemetria adicionou 0 erro, mas o débito não será chamado de gate verde. Próximo passo: corrigir a baseline por módulos, começando por `StudentPortal`, tipos do motor e configurações ES/Deno, sem misturar essa refatoração ao hotfix de telemetria. |
...
| Camada | Estado |
|---|---|
| Local | Importador MFIT 78/78 e reparo de identidade 11/11 aprovados; suíte Vitest 99 arquivos/661 testes, lint, build e gate de bundle aprovados. `tsc -b` tem a mesma baseline vermelha de 101 linhas do `origin/main`, sem erro novo neste release. |
| Último commit de código | `6ba66ce` (`feat(perf): add tenant-safe app telemetry`). |
| Integração | Branch `codex/sett-release-rc-20260826` publicada em `origin`; `origin/main` também aponta para `6ba66ce`. |
| Frontend produção | Deploy Netlify `6a9097d31bbd181fdf06a350` pronto e publicado em `www.settapp.com.br`; o chunk `assets/appPerformanceTelemetry-B-XXmZjo.js` coincide por SHA-256 com o artefato local validado. |
