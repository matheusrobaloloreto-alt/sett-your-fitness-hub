...
begin
  ...
  where audit.repair_key='bn_legacy_terms_20260909'
    ...
    and audit.before_sha256=encode(extensions.digest(
      (audit.before_enrollment||jsonb_build_object('student',audit.before_student))::text,
      'sha256'
    ),'hex');
  if v_count<>12 then raise exception 'bn_legacy_term_rollback_beforeimage_hash_mismatch expected=12 actual=%',v_count; end if;
  ...
  if exists (
    ...
    where audit.repair_key='bn_legacy_terms_20260909'
      and audit.state='applied'
      and pg_temp.bn_legacy_term_has_post_repair_activity(audit.enrollment_id,audit.applied_at)
  ) then
