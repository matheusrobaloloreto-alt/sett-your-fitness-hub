-- Dry-run wrapper for repair-bn-beatriz-aldylayne-cycle-overlaps-apply.sql.
--
-- Performs the same target updates inside one transaction, returns the changed
-- manifest, and rolls everything back. It uses a temporary audit table and
-- refuses to run if a real audit row for this repair key already exists.
...
create temporary table pg_temp.training_cycle_targeted_overlap_repair_audit (
...
) as target(action, enrollment_ref, cycle_ref, expected_start_date, expected_end_date, new_start_date, new_end_date, previous_cycle_ref);
...
with resolved as (
  ...
  select
    ...
    jsonb_build_object(
      ...
      'exercise_rows', coalesce((select sum(case when row_value.superseded_at is null and jsonb_typeof(coalesce(row_value.exercises, '[]'::jsonb)) = 'array' then jsonb_array_length(coalesce(row_value.exercises, '[]'::jsonb)) else 0 end)::integer from public.workouts row_value where row_value.cycle_id = resolved.cycle_id), 0),
      'workout_logs', (select count(distinct log.id) from public.workouts workout join public.workout_logs log on log.workout_id = workout.id where workout.cycle_id = resolved.cycle_id),
      'workout_sessions', (select count(distinct session.id) from public.workouts workout join public.workout_sessions session on session.workout_id = workout.id where workout.cycle_id = resolved.cycle_id),
    ...
    (select count(*) from public.workouts row_value where row_value.cycle_id = resolved.previous_cycle_id and row_value.superseded_at is null) as previous_workouts,
    coalesce((select sum(case when row_value.superseded_at is null and jsonb_typeof(coalesce(row_value.exercises, '[]'::jsonb)) = 'array' then jsonb_array_length(coalesce(row_value.exercises, '[]'::jsonb)) else 0 end)::integer from public.workouts row_value where row_value.cycle_id = resolved.previous_cycle_id), 0) as previous_exercise_rows
  from resolved
)
select
  deps.*,
...
with affected_enrollments as (select distinct enrollment_id from pg_temp.bn_targeted_candidates),
cycles as (select cycle.* from public.training_cycles cycle join affected_enrollments affected on affected.enrollment_id = cycle.enrollment_id),
workouts as (select workout.* from public.workouts workout join cycles cycle on cycle.id = workout.cycle_id),
workout_logs as (select log.* from public.workout_logs log join workouts workout on workout.id = log.workout_id),
workout_sessions as (select session.* from public.workout_sessions session join workouts workout on workout.id = session.workout_id)
select *
from (values
...
begin
  ...
  select encode(extensions.digest(string_agg(concat_ws('|',
    action, enrollment_ref, cycle_ref, expected_start_date::text, expected_end_date::text,
    coalesce(new_start_date::text, ''), coalesce(new_end_date::text, ''), coalesce(previous_cycle_ref, ''),
    start_date::text, end_date::text, status, coalesce(delivery_status, ''), coalesce(name, ''),
    coalesce(prescribed_offline_at::text, ''), coalesce(previous_start_date::text, ''),
    coalesce(previous_end_date::text, ''), coalesce(previous_status, ''),
    coalesce(previous_delivery_status, ''), coalesce(previous_workouts::text, ''),
    coalesce(previous_exercise_rows::text, ''), before_dependencies->>'workouts',
    before_dependencies->>'all_workouts', before_dependencies->>'exercise_rows',
    before_dependencies->>'workout_logs', before_dependencies->>'workout_sessions',
    before_dependencies->>'cycle_feedback', before_dependencies->>'ai_plan_versions',
    before_dependencies->>'ai_strength_plans', before_dependencies->>'running_plans',
    before_dependencies->>'nutrition_plans', before_dependencies->>'prescription_bundles',
    before_dependencies->>'active_prescription_bundles', before_dependencies->>'prescription_bundle_items',
    before_dependencies->>'intercycle_anamneses', before_dependencies->>'intercycle_deliveries',
    before_dependencies->>'intercycle_invites', before_dependencies->>'intercycle_waivers',
    before_dependencies->>'workout_archive_events', before_dependencies->>'cycle_clear_events',
    before_dependencies->>'carried_over_refs', before_sha256
  ...
  if exists (
    ...
    where action = 'close_active_before_sent_replacement'
      and (enrollment_ref <> '3530d245ed22' or cycle_ref <> '2024d5159013' or status <> 'active'
        or new_end_date is null or new_end_date <> previous_start_date - 1 or new_end_date < start_date
        or previous_status <> 'pending' or previous_delivery_status <> 'sent'
        or coalesce(previous_workouts, 0) <= 0 or coalesce(previous_exercise_rows, 0) <= 0
  ...
  if exists (
    ...
    where action = 'future_start_after_previous_end'
      and (status <> 'pending' or start_date <= public.current_business_date()
        or new_start_date is null or new_start_date <> previous_end_date + 1 or new_start_date > end_date
        or previous_status = 'superseded' or previous_superseded_by_cycle_id is not null
        or previous_superseded_at is not null
        or (before_dependencies->>'workout_logs')::int <> 0
        or (before_dependencies->>'workout_sessions')::int <> 0
        or (before_dependencies->>'intercycle_anamneses')::int <> 0
  ...
  if exists (
    ...
    where enrollment_ref = 'd735907ca436'
      and (cycle_ref <> '5e8114de5c79' or new_start_date <> date '2026-10-15'
...
do $residue_gate$
begin
  if to_regclass('public.training_cycle_targeted_overlap_repair_audit') is not null then
    if exists (
      select 1
...
insert into pg_temp.training_cycle_targeted_overlap_repair_audit (
  repair_key, batch_sha256, scope, action, company_id, student_id, enrollment_id,
  cycle_id, previous_cycle_id, before_cycle, before_dependencies, before_sha256
)
select
  ...
  action, company_id, student_id, enrollment_id, cycle_id, previous_cycle_id,
...
update pg_temp.training_cycle_targeted_overlap_repair_audit audit
...
begin
  ...
  from pg_temp.training_cycle_targeted_overlap_repair_audit audit
  where audit.repair_key = 'bn_beatriz_aldylayne_cycles_20260909'
  ...
  if exists (
    select 1 from pg_temp.training_cycle_targeted_overlap_repair_audit audit
    where audit.repair_key = 'bn_beatriz_aldylayne_cycles_20260909'
      and ((audit.action = 'close_active_before_sent_replacement' and ((audit.before_cycle - array['end_date', 'updated_at']) <> (audit.after_cycle - array['end_date', 'updated_at'])))
        or (audit.action = 'future_start_after_previous_end' and ((audit.before_cycle - array['start_date', 'updated_at']) <> (audit.after_cycle - array['start_date', 'updated_at']))))
  ) then
  ...
  join visible_cycles right_cycle on right_cycle.enrollment_id = left_cycle.enrollment_id
  ...
  with visible_cycles as (
    ...
    select substr(md5(left_cycle.id::text), 1, 12) as left_ref,
  ...
  select count(*), count(*) filter (where left_ref = '5e8114de5c79' or right_ref = '5e8114de5c79')
