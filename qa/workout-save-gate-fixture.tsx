...
function Fixture() {
  const [mode, setMode] = useState<"warning" | "critical">("warning");
  ...
  const save = () => {
    ...
    const draft = mode === "critical"
      ...
      : [{
        title: "Treino A - Empurrar",
        exercises: [{ exercise_id: "supino-atual", exercise_name: "Supino Inclinado com Halteres", muscle_group: "Peitoral", sets: "25" }],
      }];
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-4 p-6">
      ...
      <div className="flex gap-2">
        <button className="rounded border px-3 py-2" onClick={() => setMode("warning")}>Aviso de volume</button>
        <button className="rounded border px-3 py-2" onClick={() => setMode("critical")}>Erro crítico</button>
        <button className="rounded bg-primary px-3 py-2 text-primary-foreground" onClick={save}>Salvar Tudo</button>
      </div>
